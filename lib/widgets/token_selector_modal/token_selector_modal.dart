  ...
  static void show(
    ...
    return ZupModal.show(
      ...
      content: BlocProvider.value(
        ...
        child: TokenSelectorModal(onSelectToken: onSelectToken, onSelectTokenGroup: onSelectTokenGroup),
      ),
...
class _TokenSelectorModalState extends State<TokenSelectorModal> with DeviceInfoMixin {
  ...
  void _selectTokenGroup(TokenGroupDto group) {
    widget.onSelectTokenGroup(group);
    Navigator.of(context).pop();
  }

  Widget _buildSectionTitle(String title, {double topPadding = 20, String? tooltipMessage}) {
    final text = Text(title, style: const TextStyle(fontSize: 14, color: ZupColors.gray));
    return Padding(
  ...
  Widget _buildSliverSectionTitle(String title, {double topPadding = 20, String? tooltipMessage}) {
    return SliverToBoxAdapter(
      child: Align(
        alignment: Alignment.centerLeft,
        child: _buildSectionTitle(title, topPadding: topPadding, tooltipMessage: tooltipMessage),
      ),
    );
  }
      ...
      sliver: state.maybeWhen(
        searchSuccess: (searchResult) => SliverList.builder(
          ...
          itemBuilder: (context, index) => Padding(
            ...
            child: TokenCard(asset: searchResult[index], onClick: () => _selectToken(searchResult[index])),
          ),
        ),
    ...
    ...state.maybeWhen(
      success: (tokenList) => [
        _buildSliverSectionTitle(
          S.of(context).tokenSelectorModalTokenGroups,
          tooltipMessage: S.of(context).tokenSelectorModalTokenGroupsTooltipMessage,
        ),

        SliverPadding(
          padding: const EdgeInsets.symmetric(horizontal: 20).copyWith(bottom: 20),
          sliver: SliverList.builder(
            itemCount: tokenList.tokenGroups.length,
            itemBuilder: (context, index) {
              final group = tokenList.tokenGroups[index];

              return Padding(
                padding: _paddingBetweenListItems,
                child: TokenGroupCard(
                  key: Key("token-group-$index"),
                  group: group,
                  onClick: () => _selectTokenGroup(group),
                ),
              );
            },
          ),
