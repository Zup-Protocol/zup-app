...
void main() {
  ...
  test(
    "When calling `getTokenList` and the passed network is all networks, it should not pass the chainId param",
    () async {
      when(() => dio.get(any(), queryParameters: any(named: "queryParameters"))).thenAnswer(
        (_) async => Response(data: TokenListDto.fixture().toJson(), statusCode: 200, requestOptions: RequestOptions()),
      );

      await sut.getTokenList(AppNetworks.allNetworks);

      verify(() => dio.get("/tokens/list", queryParameters: {})).called(1);
    },
  );

  test(
    "When calling `getTokenList` and the passed network is not all networks, it should not the correct chainId param",
    () async {
      ...
      await sut.getTokenList(AppNetworks.scroll);

      verify(() => dio.get("/tokens/list", queryParameters: {"chainId": network.chainId})).called(1);
    },
  );
  ...
  test("When calling `getTokenPrice` it should call the correct endpoint with correct params", () async {
    ...
    when(() => dio.get(any(), queryParameters: any(named: "queryParameters"))).thenAnswer(
      (_) async => Response(data: TokenPriceDto.fixture().toJson(), statusCode: 200, requestOptions: RequestOptions()),
    );
  ...
  test("When calling `getTokenPrice` it should correctly parse the response", () async {
    ...
    when(() => dio.get(any(), queryParameters: any(named: "queryParameters"))).thenAnswer(
      (_) async => Response(data: tokenPriceDto.toJson(), statusCode: 200, requestOptions: RequestOptions()),
    );
