...
void main() {
  ...
  zGoldenTest(
    ...
    final tokenA = selectedNetwork.defaultToken;

    when(() => tokensRepository.getTokenList()).thenAnswer(
      (_) async => TokenListDto.fixture().copyWith(
        mostUsedTokens: [tokenA!],
        popularTokens: [tokenA],
        userTokens: [tokenA],
      ),
    );
  ...
  zGoldenTest(
    ...
    const tokenAName = "Token1";
    const tokenBName = "Token2";
    ...
    await tester.tap(find.byKey(const Key("token-a-selector")));
    await tester.pumpAndSettle();
    await tester.tap(find.text(tokenAName));
    await tester.pumpAndSettle();

    await tester.tap(find.byKey(const Key("token-b-selector")));
    await tester.pumpAndSettle();
    await tester.tap(find.text(tokenBName));
    await tester.pumpAndSettle();
  });
}
